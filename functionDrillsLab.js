...
let bestMovie = (mov) => {
  return `${mov} is the best movie ever!`
}

let movieOpinion = bestMovie('Tower Heist')
console.log(movieOpinion);
...
function theEliminator(cont, lsr){
...
console.log(toUpp(sampleString));
...
console.log(emailCheck('   hahspiderpig@gmail.com   '));
...
function frgBuyer(gold){
  ...
  for(i = 0; i <= totalGold; i++){
    if(gold > 2){
      ...
      return `You bought ${i} total frogs!`
    }
  }
}

let totalFrogs = frgBuyer(4)
...
//I didn't have that bug. I guess I'm just a jsNinja
